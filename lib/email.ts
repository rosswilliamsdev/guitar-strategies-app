    ...
    const result = await withRetry(async () => {
      ...
      if (emailResult.error) {
        const error = new Error(
          emailResult.error.message || "Email sending failed"
        ) as Error & { status?: number };
        // Attach status code if available for retry logic
        error.status =
          emailResult.error.name === "rate_limit_exceeded" ? 429 : 500;
        throw error;
      }
...
function logEmailError(
  ...
  // Type assertion: we handle any shape of error object safely
...
export function createChecklistCompletionEmail(
  ...
  const content = `
    ...
    ${
      teacherName
        ? `<p>Your teacher ${teacherName} will be proud of your accomplishment!</p>`
        : ""
    }
  ...
  return createBaseTemplate(
    content,
    "Checklist Completed! - Guitar Strategies"
  );
}
...
export function createOverdueInvoiceEmail(
  ...
  return createBaseTemplate(
    content,
    `Overdue Invoice ${invoiceNumber} - Guitar Strategies`
  );
}
...
export function createLessonBookingEmail(
  ...
  const content = `
    ...
    <p>Hi ${studentName},</p>
    <p>Great news! Your guitar lesson${
      isRecurring ? " series" : ""
    } has been booked successfully.</p>
    ...
    <div class="info-box">
      ...
      ${
        isRecurring
          ? "• You can cancel individual lessons or the entire recurring series from your dashboard"
          : ""
      }
    </div>
    
    <p>We're excited for your upcoming lesson${
      isRecurring ? "s" : ""
    }! If you have any questions, please contact ${teacherName} directly.</p>
  ...
  return createBaseTemplate(
    content,
    `Lesson Booked${isRecurring ? " Series" : ""} - Guitar Strategies`
  );
}
...
export function createInvoiceEmail(
  ...
  return createBaseTemplate(
    content,
    `Invoice ${invoiceNumber} - Guitar Strategies`
  );
}
...
export function createPasswordResetEmail(
  ...
  return createBaseTemplate(
    content,
    "Password Reset Request - Guitar Strategies"
  );
}
...
export function createStudentInvitationEmail(
  ...
  return createBaseTemplate(
    content,
    `Invitation from ${teacherName} - Guitar Strategies`
  );
}
