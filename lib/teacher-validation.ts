...
/**
 * Validate all teachers in the system and return a report
 */
export async function validateAllTeachers(): Promise<{
  totalTeachers: number;
  completeProfiles: number;
  incompleteProfiles: number;
  commonIssues: Record<string, number>;
  teachersWithIssues: Array<{
    id: string;
    name: string;
    email: string;
    issues: string[];
  ...
  try {
    ...
    const teachersWithIssues: Array<{
      id: string;
      name: string;
      email: string;
      issues: string[];
    ...
    for (const teacher of teachers) {
      if (!teacher.teacherProfile) {
        incompleteProfiles++;
        teachersWithIssues.push({
          id: teacher.id,
          name: teacher.name || 'Unknown',
          email: teacher.email || 'Unknown',
          issues: ['No teacher profile created'],
        });
        continue;
      }

      const validation = await validateTeacherProfile(teacher.teacherProfile.id);
      ...
      if (validation.isComplete) {
        ...
        for (const field of validation.missingFields) {
          commonIssues[field] = (commonIssues[field] || 0) + 1;
        }

        // Add to teachers with issues list
        if (validation.errors.length > 0 || validation.missingFields.length > 0) {
          teachersWithIssues.push({
            id: teacher.id,
            name: teacher.name || 'Unknown',
            email: teacher.email || 'Unknown',
            issues: [...validation.errors, ...validation.missingFields.map(f => `Missing: ${f}`)],
          });
        }
      }
    }
    ...
    console.error("Error validating all teachers:", error);
    throw error;
  }
}