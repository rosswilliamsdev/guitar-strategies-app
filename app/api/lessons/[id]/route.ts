...
export async function PUT(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    if (body.homework !== undefined) {
      updateData.homework = body.homework
        ? sanitizeRichText(body.homework)
        : null;
    }
    if (body.progress !== undefined) {
      updateData.progress = body.progress
        ? sanitizeRichText(body.progress)
        : null;
    }
    if (body.nextSteps !== undefined) {
      updateData.nextSteps = body.nextSteps
        ? sanitizeRichText(body.nextSteps)
        : null;
    }
    ...
    if (body.focusAreas !== undefined) {
      updateData.focusAreas = body.focusAreas
        ? sanitizePlainText(body.focusAreas)
        : null;
    }
    if (body.songsPracticed !== undefined) {
      updateData.songsPracticed = body.songsPracticed
        ? sanitizePlainText(body.songsPracticed)
        : null;
    }
    ...
    if (typeof expectedVersion !== "number") {
      return NextResponse.json(
        {
          error:
            "Version required for optimistic locking. Please refresh and try again.",
        },
        { status: 400 },
      );
    }
    ...
    await retryOptimisticUpdate(async () => {
      try {
        ...
        if (error instanceof OptimisticLockingError) {
          throw new Error(
            `Lesson was modified by another user. Current version: ${error.currentVersion}, your version: ${error.attemptedVersion}. Please refresh and try again.`,
          );
        }
        throw error;
      }
