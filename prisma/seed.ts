...
async function main() {
  ...
  try {
        ...
        data: {
          ...
          studentProfile: {
            create: {
              ...
              goals:
                "Learn to play acoustic guitar and improve fingerpicking technique. Interested in folk and indie music styles.",
              instrument: "guitar",
    ...
    if (teacherUser.teacherProfile) {
      log.info(
        `│ 💰 Rate: $${(teacherUser.teacherProfile.hourlyRate! / 100).toFixed(2)}/hour${" ".repeat(18)} │`,
      );
    }
    log.info("└─────────────────────────────────────────┘");
